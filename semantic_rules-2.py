...
def fix_object_name(name: str):
    if not name:
        return name
    return SEMANTIC_NAME_FIXES.get(name.lower(), name)


def fix_field_name(name: str):
    if not name:
        return name
    return SEMANTIC_FIELD_FIXES.get(name.lower(), name.lower())
...
def apply_semantic_rules(schema: dict):
    ...
    for action in schema.get("actions", []):
        for key in ["target", "from", "to", "object"]:
            if key in action:
                action[key] = object_rename_map.get(action[key], action[key])
        if "entities_involved" in action:
            action["entities_involved"] = [
                object_rename_map.get(e, e) for e in action["entities_involved"]
            ]
