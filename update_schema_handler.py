...
def _ensure_schema_structure(schema: Optional[Dict]) -> Dict:
    """
    Garante que o schema tem sempre as três chaves raiz com valores válidos.
    
    PORQUÊ: setdefault("objects", []) NÃO substitui None explícito — apenas
    adiciona a chave se AUSENTE. Se a cache devolver {"objects": None},
    o None sobrevive e _build_context_string crasha ao iterar.
    parse_blueprint trata None explícito, tipos errados e chaves ausentes.
    """
    from models import parse_blueprint
    return parse_blueprint(schema if isinstance(schema, dict) else {}).to_dict()
