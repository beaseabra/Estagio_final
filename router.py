...
# Palavras-chave que indicam alteração de algo existente
_UPDATE_KEYWORDS = {
    "adiciona", "adicionar",
    "remove", "remover",
    "apaga", "apagar",
    ...
    "mete", "coloca",
    "retira", "tirar",
    "campo", "fields", "atributo",
    "tipo", "relação", "relacao",
    "workspace", "ação", "acao"
}
...
_OBJECT_PATTERNS = [
    ...
    r"\bgerar\s+objeto\b",
    r"\bobjeto\s+\w+\s+com\s+os\s+campos\b",
    r"\bobject\s+\w+\s+with\s+fields\b",
]


# Pedidos de workspace isolado
_WORKSPACE_PATTERNS = [
...
def classify_prompt(prompt: str, current_schema: Dict[str, Any]) -> str:
    ...
    # =========================
    # Mesmo que exista schema ativo, se o utilizador disser claramente
    # "cria um sistema", assumimos que quer começar uma geração de sistema.

    if _matches_any(prompt_lower, _SYSTEM_PATTERNS):
        return "CREATE_SYSTEM"

    # =========================
    # 2. Se há schema ativo, quase tudo que mexe em estrutura é UPDATE_SCHEMA
    # =========================
    # Exemplos:
    # "remove o campo stock"
    # "troca preco por valor_unitario"
    # "adiciona um objeto Cliente"
    # "cria uma relação entre Projeto e Tarefa"
    ...
    # =========================
    # 3. Sem schema ativo: objeto simples
    # =========================
    ...
    # =========================
    # 4. Sem schema ativo: workspace simples
    # =========================
