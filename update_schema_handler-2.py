...
def _parse_field_list(raw: str) -> List[Dict[str, str]]:
    ...
    for part in parts:
        part = re.sub(r"^(?:campo|campos)\s+", "", part, flags=re.IGNORECASE).strip()
        ...
        if ":" in part:
            ...
            match = re.match(
                ...
                r"(string|integer|float|boolean|date|datetime|text|inteiro|texto|decimal|numero|número|data|booleano)$",
                part,
                re.IGNORECASE,
            )
            if match:
...
def _rule_based_operations(user_prompt: str, bp: BlueprintModel) -> List[Dict[str, Any]]:
    ...
    remove_field_pattern = (
        ...
        r"(?:\s+(?:do|da|de|no|na|em)\s+(?:(?:objeto|tabela)\s+)?([a-zA-ZÀ-ÿ0-9_]+))?"
    )
    ...
    # 3. RETYPE_FIELD
    retype_field_patterns = [
        (
            r"(?:muda|mudar|altera|alterar|atualiza|atualizar)\s+"
            r"(?:o\s+)?tipo\s+(?:do\s+)?campo\s+([a-zA-ZÀ-ÿ0-9_]+)\s+"
            r"(?:para|como)\s+([a-zA-ZÀ-ÿ0-9_]+)"
            r"(?:\s+(?:no|na|em|do|da|de)\s+(?:(?:objeto|tabela)\s+)?([a-zA-ZÀ-ÿ0-9_]+))?"
        ),
        (
            r"(?:muda|mudar|altera|alterar|atualiza|atualizar)\s+"
            r"(?:o\s+)?campo\s+([a-zA-ZÀ-ÿ0-9_]+)\s+"
            r"(?:para|como)\s+([a-zA-ZÀ-ÿ0-9_]+)"
            r"(?:\s+(?:no|na|em|do|da|de)\s+(?:(?:objeto|tabela)\s+)?([a-zA-ZÀ-ÿ0-9_]+))?"
        ),
    ]

    for pattern in retype_field_patterns:
        for match in re.finditer(pattern, low, re.IGNORECASE):
            field_name = _norm_token(match.group(1))
            new_type = _normalize_type(match.group(2))
            explicit_obj = match.group(3) if len(match.groups()) >= 3 else None
            obj_name = _find_object_for_field(bp, field_name, explicit_obj)
    ...
    add_field_pattern = (
        ...
        r"(?:o\s+)?campo\s+([a-zA-ZÀ-ÿ0-9_]+)"
        r"(?:\s+(?:ao|à|a|no|na|em|para)\s+(?:(?:objeto|tabela)\s+)?([a-zA-ZÀ-ÿ0-9_]+))?"
    ...
    for match in re.finditer(add_field_pattern, low, re.IGNORECASE):
        ...
        field_name = _norm_token(raw_field)
        explicit_obj = match.group(2)
    ...
    add_object_pattern = (
        ...
        r"(.+?)\s+"
        r"com\s+(?:(?:os|as)\s+)?(?:campo|campos)?\s*"
        r"(.+?)(?=$|\s+e\s+(?:adiciona|remove|renomeia|muda|cria|apaga|elimina|retira|relaciona|liga)\b)"
    )

    for match in re.finditer(add_object_pattern, text, re.IGNORECASE):
        object_name = re.sub(r"\s+", " ", str(match.group(1)).strip())
        fields_raw = match.group(2).strip()

        fields_raw = re.sub(r"^(?:campo|campos)\s+", "", fields_raw, flags=re.IGNORECASE).strip()
    ...
    add_relation_patterns = [
        ...
        (
            r"(?:cria|criar|adiciona|adicionar)\s+"
            r"(?:uma\s+)?(?:relação|relacao)\s+"
            r"(.+?)\s+(?:com|e|a|ao|à)\s+(.+?)"
            r"(?:\s+(?:como|do\s+tipo|tipo)\s+(ONE_TO_MANY|MANY_TO_MANY))?"
            r"(?=$|\s+e\s+(?:adiciona|remove|renomeia|muda|cria|apaga|elimina|retira|relaciona|liga)\b)"
        ),
        (
            r"(?:liga|ligar|relaciona|relacionar|associa|associar)\s+"
            r"(.+?)\s+(?:a|ao|à|com|e)\s+(.+?)"
            r"(?:\s+(?:como|do\s+tipo|tipo)\s+(ONE_TO_MANY|MANY_TO_MANY))?"
            r"(?=$|\s+e\s+(?:adiciona|remove|renomeia|muda|cria|apaga|elimina|retira|relaciona|liga)\b)"
        ),
    ...
    for pattern in add_relation_patterns:
        for match in re.finditer(pattern, text, re.IGNORECASE):
            from_raw = re.sub(r"\s+", " ", str(match.group(1)).strip())
            to_raw = re.sub(r"\s+", " ", str(match.group(2)).strip())

            from_obj = _resolve_object_name(from_raw)
            to_obj = _resolve_object_name(to_raw)
    ...
    remove_relation_pattern = (
        ...
        r"([a-zA-ZÀ-ÿ0-9_]+)\s+(?:e|com|a|ao|à)\s+([a-zA-ZÀ-ÿ0-9_]+)"
    )
...
def run_regression_tests() -> Dict[str, Any]:
    ...
    ops11 = _rule_based_operations("cria um objeto Cliente com campo nome, email e telefone", bp7)
    results["caso_14_rule_add_object_campo_singular"] = {
        "passou": bool(
            ops11
            and ops11[0]["op"] == "ADD_OBJECT"
            and ops11[0]["object"]["name"] == "Cliente"
            and [f["name"] for f in ops11[0]["object"]["fields"]] == ["nome", "email", "telefone"]
        ),
        "ops": ops11,
    }

    ops12 = _rule_based_operations("cria objeto Cliente com nome,email,telefone", bp7)
    results["caso_15_rule_add_object_sem_palavra_campos"] = {
        "passou": bool(
            ops12
            and ops12[0]["op"] == "ADD_OBJECT"
            and [f["name"] for f in ops12[0]["object"]["fields"]] == ["nome", "email", "telefone"]
        ),
        "ops": ops12,
    }
    ...
    bp8 = parse_blueprint({
        "objects": [
            {"name": "Cliente", "fields": [{"name": "nome", "type": "string"}]},
            {"name": "Encomenda", "fields": [{"name": "codigo", "type": "string"}]},
        ],
        "relations": [],
        "workspaces": [],
        "actions": [],
    ...
    ops13 = _rule_based_operations("adiciona campo estado em Encomenda", bp8)
    results["caso_16_rule_add_field_em_objeto"] = {
        "passou": bool(
            ops13
            and ops13[0]["op"] == "ADD_FIELD"
            and ops13[0]["object"] == "Encomenda"
            and ops13[0]["field"]["name"] == "estado"
        ),
        "ops": ops13,
    }

    ops14 = _rule_based_operations("adiciona o campo aprovado à tabela Encomenda como boolean", bp8)
    results["caso_17_rule_add_field_tabela_tipo"] = {
        "passou": bool(
            ops14
            and ops14[0]["op"] == "ADD_FIELD"
            and ops14[0]["object"] == "Encomenda"
            and ops14[0]["field"]["name"] == "aprovado"
            and ops14[0]["field"]["type"] == "boolean"
        ),
        "ops": ops14,
    }

    ops15 = _rule_based_operations("relaciona Cliente com Encomenda", bp8)
    results["caso_18_rule_relaciona_objetos"] = {
        "passou": bool(
            ops15
            and ops15[0]["op"] == "ADD_RELATION"
            and ops15[0]["relation"]["from"] == "Cliente"
            and ops15[0]["relation"]["to"] == "Encomenda"
        ),
        "ops": ops15,
    }

    ops16 = _rule_based_operations("cria relação Cliente Encomenda", bp8)
    results["caso_19_rule_relacao_sem_entre"] = {
        "passou": bool(
            ops16
            and ops16[0]["op"] == "ADD_RELATION"
            and ops16[0]["relation"]["from"] == "Cliente"
            and ops16[0]["relation"]["to"] == "Encomenda"
        ),
        "ops": ops16,
    }
