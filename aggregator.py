...
def aggregate_blueprint(
    ...
    for rel in normalized_relations:
        ...
        # CORREÇÃO FASE 1: Verificação robusta de FK duplicadas (normalização aplicada)
        existing_fields = {normalize_field_name(f["name"]) for f in object_map[from_obj]["fields"]}
        ...
        if fk not in existing_fields:
            ...
            print(f"[aggregator] INFO: FK '{fk}' já existia em '{from_obj}', injecção duplicada evitada.")
